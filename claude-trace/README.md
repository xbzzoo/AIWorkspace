...
用 [claude-trace](https://github.com/badlogic/lemmy/tree/main/apps/claude-trace) 在 Node 进程级别 hook 住 Claude Code CLI 的 HTTPS 栈，把每一对 request/response 落成 JSON Lines，从**流量层**还原"一次 skill 调用"（实验指令：`/xx_code_view git diff`）的完整时序。

> 注：claude-trace 当前只兼容 Claude Code 版本 < 2.1.112。

## 核心结论

- 一次 `/xx_code_view git diff` 在 HTTP 层留下 **34 条** request/response，其中只有 **11 条** 是 `/v1/messages`，其余 23 条是配置 / 服务发现 / 遥测等会话外围流量。
...
| `detailed-v1-message-report.html` | 抓包导出的详细 trace 报告（单文件，约 7 MB，浏览器打开）；密钥/Cookie 已 `[REDACTED]`，品牌字样已脱敏 |
